...
# stream test status back. RAFT only acts on this block if its
# version supports it; older RAFTs ignore it.
monitor:
  enabled: true
  allocator_url:       "http://allocator.lab:5000"
  test_execution_id:    null    # filled in by /start_test before the run
  heartbeat:
    endpoint:          "/test_heartbeat"
    interval_seconds:  60
    timeout_seconds:   600       # "hung" after this without a beat
...
The allocator already has the *receiving* side of this:
`POST /start_test`, `POST /test_heartbeat`, `POST /end_test`, plus
the `TestExecution` model with `last_heartbeat`, `heartbeat_timeout`,
and the `GET /device/<id>/box_status` polling endpoint that reports
`active_test.heartbeat_age_seconds`.
...
- **Always optional.** If RAFT is older or the engineer wants to run
  silent, drop the `monitor:` block (or `monitor.enabled: false`)
  and nothing calls back. Box state stays "allocated" through the
  test run rather than transitioning to "busy".
...
- **Hung detection is server-side.** Heartbeat timeout
  ...
  beating.
- **Failure modes are graceful.** Callback failures (network drop)
  must not fail the test — RAFT logs and continues. The allocator
  will mark the test `hung` from its side; the engineer's test
  output is the source of truth on success/failure.

This is the shape the user asked to "be in the allocator design
already." It's now documented here and in
[architecture.md §4.2](./architecture.md) (added in the same commit).
...
3. **Monitor block honour** — new module `framework/core/monitor.py`:
   - On `testControl` start, if `monitor.enabled` is true, POST to
   ...
   - All callback failures: log warning, do not fail the test.
